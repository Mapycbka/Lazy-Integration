...
import { dequal } from '../../dequal/dist/index.js';
import makeCancellablePromise from '../../make-cancellable-promise/dist/index.js';
import makeEventProps from '../../make-event-props/dist/index.js';
import * as pdf from '../../pdfjs-dist/build/pdf.js';
import { getDocument } from '../../pdfjs-dist/build/pdf.js';
import invariant from '../../tiny-invariant/dist/esm/tiny-invariant.js';
...
import { isDataURI, dataURItoByteString, displayCORSWarning, isArrayBuffer, isBrowser, isBlob, loadFromFile, cancelRunningTask } from './shared/utils.js';

const { PDFDataRangeTransport } = pdf;
...
const Document = forwardRef(function Document({ children, className, error = 'Failed to load PDF file.', externalLinkRel, externalLinkTarget, file, inputRef, imageResourcesPath, loading = 'Loading PDF…', noData = 'No PDF file specified.', onItemClick, onLoadError: onLoadErrorProps, onLoadProgress, onLoadSuccess: onLoadSuccessProps, onPassword = defaultOnPassword, onSourceError: onSourceErrorProps, onSourceSuccess: onSourceSuccessProps, options, renderMode, rotate, scale, ...otherProps }, ref) {
    ...
    const { value: pdf$1, error: pdfError } = pdfState;
    ...
    function onLoadSuccess() {
        ...
        if (onLoadSuccessProps) {
            onLoadSuccessProps(pdf$1);
        }
        pages.current = new Array(pdf$1.numPages);
        linkService.current.setDocument(pdf$1);
    }
    /**
    ...
    const childContext = useMemo(() => ({
        ...
        scale,
        unregisterPage,
    }), [imageResourcesPath, onItemClick, pdf$1, registerPage, renderMode, rotate, scale, unregisterPage]);
    const eventProps = useMemo(() => makeEventProps(otherProps, () => pdf$1), 
    ...
    function renderChildren() {
        ...
        return (jsx(DocumentContext.Provider, { value: childContext, children: resolvedChildren }));
    }
    function renderContent() {
        if (!file) {
            return jsx(Message, { type: "no-data", children: typeof noData === 'function' ? noData() : noData });
        }
        if (pdf$1 === undefined || pdf$1 === null) {
            return (jsx(Message, { type: "loading", children: typeof loading === 'function' ? loading() : loading }));
        }
        if (pdf$1 === false) {
            return jsx(Message, { type: "error", children: typeof error === 'function' ? error() : error });
        }
        return renderChildren();
    }
    return (jsx("div", { className: clsx('react-pdf__Document', className), 
        // Assertion is needed for React 18 compatibility
        ref: inputRef, ...eventProps, children: renderContent() }));
});
var Document$1 = Document;

export { Document$1 as default };
